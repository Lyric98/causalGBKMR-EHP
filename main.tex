...
Table~\ref{tab:fastbkmr_time} reports per-model wall-clock time across the four methods, averaged over the three replicates. With 10-core parallelisation, the fastBKMR engine fits the full-cohort model in approximately $50$ minutes per model, compared with $7$ minutes for the single-core BKMR fit on $n = 500$ and $80$ minutes for the single-core BKMR fit on $n = 2{,}000$. At full cohort scale on a single core, standard BKMR takes approximately $12$ hours per model fit, which is the practical motivation for either subsampling or switching to the fastBKMR subset-aggregation strategy: compared with the single-core BKMR fit at $n = 2{,}000$, full-cohort standard BKMR uses three times as much data at roughly $10\times$ the wall time, whereas fastBKMR reaches the same full-cohort data scale at $\sim 50$ minutes per fit when ten cores are available. Per-model timings within each engine are nearly insensitive to the number of kernel inputs (BKMR with 50 knots: $6.4$ to $7.2$ minutes from 4 to 14 inputs; fastBKMR with the longer chain: $49.5$ to $49.8$ minutes across the same range), so model size is not the binding constraint in this regime. Translating per-model timings to a complete g-formula run that fits the two mediator models and the outcome model yields approximately $21$ minutes for the standard BKMR pipeline on $n = 500$, $4$ hours for the standard BKMR pipeline on $n = 2{,}000$, $36$ hours for the standard BKMR pipeline on the full $n = 6{,}000$, and $2.5$ hours for the fastBKMR pipeline on the full $n = 6{,}000$.
...
$^{\dagger}$ BKMR full-cohort cells show a single completed replicate; the remaining seven fits hit the 12-hour wall limit on the \texttt{short} partition and are being re-run on the \texttt{burst} partition (Slurm job $7995182$). The single-replicate timing is $\approx 12$ hours per fit, consistent with super-linear scaling of \pkg{kmbayes()} in $n$ at fixed knots.}
...
BKMR (standard)    & 6{,}000 & $\approx 719^{\dagger}$ & $\approx 719^{\dagger}$ & ---$^{\dagger}$ \\
