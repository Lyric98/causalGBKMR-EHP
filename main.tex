...
For each of three model types in the g-formula sequence (mediator at $t=1$ with 4 kernel inputs, mediator at $t=2$ with 9 inputs, and outcome model with 14 inputs), we fit three configurations: (i) standard BKMR with $n = 500$ random subsample and 50 knots; (ii) standard BKMR with $n = 2{,}000$ random subsample and 50 knots; and (iii) fastBKMR on the full $n = 6{,}000$ sample using 10 random subsets of size $\sim 600$, fit in parallel on 10 cores via \code{skmbayes(parallel = TRUE)}. To make the comparison fair, all engines used $15{,}000$ MCMC iterations and \code{varsel = TRUE}; the fastBKMR runs additionally set \code{est.h = FALSE} so that the spatial smoothing variance is estimated post hoc rather than at each iteration. Two baseline covariates (sex and waist circumference at baseline) entered each model linearly. Knot approximation is not currently supported in \code{skmbayes()}, so the fastBKMR subsets fit the exact Gaussian kernel. Each configuration was replicated three times.

Table~\ref{tab:fastbkmr_time} reports per-model wall-clock time across the three methods, averaged over the three replicates. With 10-core parallelisation, the fastBKMR engine fit the full-cohort model in approximately 50 minutes per model, compared with 7 minutes for the single-core BKMR fit on $n = 500$ and 80 minutes for the single-core BKMR fit on $n = 2{,}000$. Per-model timings were nearly insensitive to the number of kernel inputs for all three engines (BKMR with 50 knots: 6.4 to 7.2 minutes from 4 to 14 inputs; fastBKMR with the longer chain: 49.5 to 49.8 minutes across the same range), so model size is not the binding constraint in this regime. Translating per-model timings to a complete g-formula run that fits the two mediator models and the outcome model yields approximately 21 minutes for the standard BKMR pipeline on $n = 500$, four hours for the standard BKMR pipeline on $n = 2{,}000$, and 2.5 hours for the fastBKMR pipeline on the full $n = 6{,}000$. Compared with BKMR on $n = 2{,}000$, the fastBKMR pipeline uses three times as much data at roughly $60\%$ of the standard-BKMR wall time when ten cores are available.
...
\begin{table}[h]
\centering
\caption{Wall-clock time per model fit (minutes, mean $\pm$ SD across three replicates) for the standard \pkg{bkmr} engine on random subsamples of size $n = 500$ and $n = 2{,}000$ and for the \pkg{fbkmr} engine on the full $n = 6{,}000$ sample with 10-core parallel subset fitting. Mediator at $t=1$ uses 4 kernel inputs, mediator at $t=2$ uses 9 inputs, and the outcome model uses 14 inputs. All engines use $15{,}000$ MCMC iterations and component-wise variable selection; the standard BKMR runs use 50 knots; the fastBKMR runs use 10 subsets of size $\sim 600$ each with no knot approximation. Numbers are from Scenario 3d (continuous outcome, $n = 6{,}000$); a controlled rerun under a binary outcome differed by at most $\pm 5\%$ per cell.}
