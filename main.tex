...
\begin{table}[h]
\centering
\caption{Engine support for \code{gbkmr\_run()} as a function of outcome type and time-varying confounder type. The standard \pkg{bkmr} engine supports every combination, using probit BKMR for binary $Y$ and binary $L_t$ and Bernoulli sampling of counterfactual $L_t$ during Monte Carlo g-computation. The \pkg{fbkmr} engine is restricted to fully continuous problems and raises a hard error otherwise; the default \code{engine = "auto"} setting falls back to \pkg{bkmr} when any binary variable is detected.}
\label{tab:engine_support}
\begin{tabular}{llll}
\toprule
Outcome $Y$ & Confounder $L_t$ & \code{engine = "bkmr"} & \code{engine = "fastbkmr"} \\
\midrule
continuous & all continuous & Supported (Gaussian)                              & Supported (Gaussian) \\
binary     & all continuous & Supported (probit $Y$)                            & Hard error \\
continuous & any binary     & Supported (probit $L_t$, Bernoulli MC)            & Hard error \\
binary     & any binary     & Supported (probit $Y$, probit $L_t$, Bernoulli MC) & Hard error \\
\bottomrule
\end{tabular}
\end{table}
