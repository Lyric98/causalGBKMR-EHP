...
Standard BKMR can become computationally expensive when the sample size is large or when the longitudinal structure requires many sequential mediator models. For analyses in which the outcome and all time-varying confounders are continuous, \pkg{CausalBKMR} can use \pkg{fbkmr} through the \code{engine = "fastbkmr"} option. The fastBKMR path implements the median-posterior subset-aggregation strategy of \citet{sonabend_fastbkmr_2026}, which fits Gaussian-kernel BKMR independently on multiple data subsets and combines posterior summaries through the median of subset posteriors.

\subsection*{Engine support and outcome--confounder types}
The two engines do not have identical support for outcome and time-varying-confounder types. Table~\ref{tab:engine_support} summarises the current policy. The standard \pkg{bkmr} engine accepts every combination of continuous or binary outcome and continuous or binary time-varying confounder. A binary outcome triggers a probit BKMR outcome model and reporting of the causal effect as a risk difference on the probability scale. A binary time-varying confounder triggers a probit BKMR mediator model for that confounder and Bernoulli sampling of its counterfactual values during Monte Carlo g-computation, so the modelled distribution of $L_t$ stays consistent with its observed scale. The \pkg{fbkmr} engine is currently restricted to fully continuous problems: \code{gbkmr\_run()} raises a hard error if \code{engine = "fastbkmr"} is requested together with any binary $Y$ or any binary $L_t$, because the median-posterior aggregation strategy has been validated for Gaussian kernels only. The default \code{engine = "auto"} setting falls back to \code{engine = "bkmr"} as soon as a binary outcome or a binary $L_t$ is detected; users who want to force one engine can do so explicitly.
...
For binary outcomes, users specify \code{outcome\_type = "binary"}; the standard BKMR engine fits a probit BKMR outcome model and reports the causal effect on the probability scale as a risk difference. Binary time-varying confounders are detected automatically from the prepared data and trigger a probit BKMR mediator model with Bernoulli sampling of counterfactual values during Monte Carlo g-computation, matching the policy in Table~\ref{tab:engine_support}. Requesting \code{engine = "fastbkmr"} with any binary $Y$ or any binary $L_t$ stops the command with an explicit error; the default \code{engine = "auto"} setting transparently falls back to the standard \pkg{bkmr} engine in those cases.

The command also handles \code{time\_points = 1} as a degenerate case: when there are no follow-up visits, the mediator sequence is empty and \code{gbkmr\_run()} reduces to an outcome-only BKMR fit on the baseline exposures and covariates, which still goes through the audit and posterior-summary plumbing. This makes the same command interface usable for cross-sectional mixture analyses without requiring the user to switch packages.
...
A single call to \code{gbkmr\_run()} performs the input audit, fits the sequential BKMR mediator and outcome models, runs Monte Carlo g-computation, and returns a \code{gbkmr\_results} object.
...
With \code{engine = "auto"}, a continuous outcome, and all-continuous time-varying confounders, the command routes to the standard \pkg{bkmr} engine and fits three sequential models in this analysis: waist and glucose mediators at the appropriate visits, followed by the outcome model on the full exposure-confounder history. MCMC is run for $24{,}000$ iterations and $K = 1{,}000$ counterfactual trajectories are drawn per retained posterior sample. Had any time-varying confounder been binary, the command would have selected probit BKMR mediator models and Bernoulli sampling automatically (Table~\ref{tab:engine_support}).
...
\begin{table}[h]
\centering
\caption{Wall-clock time per model fit (minutes, mean $\pm$ SD across three replicates) for each engine configuration in Table~\ref{tab:fastbkmr_config}. Mediator at $t=1$ uses 4 kernel inputs, mediator at $t=2$ uses 9 inputs, and the outcome model uses 14 inputs. Numbers are from Scenario 3d (continuous outcome, $n = 6{,}000$); a controlled rerun under a binary-outcome DGP (Scenario 3c) differed by at most $\pm 5\%$ per cell for the three engines for which 3c data exist.}
